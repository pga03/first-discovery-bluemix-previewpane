...
(function ($, fluid) {
    ...
    gpii.tests.testRendering = function (that) {
        ...
        jqUnit.assertEquals("The description should be rendered correctly"+" Oh Yea, Brother!", expectedContent, that.locate("message").text());
    };
    ...
    fluid.defaults("gpii.tests.firstDiscovery.panel.goodbye", {
        gradeNames: ["gpii.firstDiscovery.panel.goodbye"],
        messageBase: {
            "message": "<p>Goodbye!</p><p>Don't come back!.</p>"
        }
    });

    fluid.defaults("gpii.tests.goodbyePanel", {
        gradeNames: ["fluid.test.testEnvironment"],
        components: {
            goodbye: {
                type: "gpii.tests.firstDiscovery.panel.goodbye",
                container: ".gpiic-fd-goodbye"
            },
            goodbyeTester: {
                type: "gpii.tests.goodbyeTester"
            }
        }
    });

    fluid.defaults("gpii.tests.goodbyeTester", {
        gradeNames: ["fluid.test.testCaseHolder"],
        modules: [{
            name: "Tests the Goodbye component",
            tests: [{
                expect: 1,
                name: "Initialization",
                sequence: [{
                    func: "{goodbye}.refreshView"
                }, {
                    listener: "gpii.tests.testRendering",
                    args: ["{goodbye}"],
                    event: "{goodbye}.events.afterRender"
                }]
            }]
        }]
    });
