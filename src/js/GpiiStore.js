...
(function ($, fluid) {
     ...
     */
    fluid.defaults("gpii.firstDiscovery.gpiiStore", {
        gradeNames: ["fluid.prefs.dataSource", "autoInit"],
        url: "http://preferences.gpii.net/user/firstDiscovery-user-1",
        gpiiEntry: "http://registry.gpii.org/applications/gpii.firstDiscovery",
        invokers: {
            get: {
                funcName: "gpii.firstDiscovery.gpiiStore.get",
                args: ["{that}.options"]
            },
            set: {
                funcName: "gpii.firstDiscovery.gpiiStore.set",
                args: ["{arguments}.0", "{that}.options"]
            }
        }
    });

    gpii.firstDiscovery.gpiiStore.get = function (settings) {
        var gpiiModel;
    ...
    gpii.firstDiscovery.gpiiStore.set = function (model, settings) {
        var dataToSave = {};
    ...
    fluid.defaults("gpii.firstDiscovery.gpiiSettingsStore", {
        gradeNames: ["autoInit", "fluid.globalSettingsStore"],
        storeType: "gpii.firstDiscovery.gpiiStore",
        distributeOptions: [{
